...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Dados do usuário (normalmente viriam de um contexto ou hook)
  const userData = {
    name: 'Professor(a)',
        ...
        <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-purple-50">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Badge className={`text-xs px-2 py-1 ${getPlanBadgeColor()}`}>
                {getPlanDisplayName()}
              </Badge>
            </div>
          </div>
          <button
            ...
            className="w-full text-left text-xs text-blue-600 hover:text-blue-800 font-medium transition-colors"
          >
            Ver perfil completo →
          </button>
        </div>
        ...
        {canAccessSchool() && (
          <div className="absolute -top-10 right-4">
            <button
              ...
              className="flex items-center justify-center bg-emerald-500 text-white rounded-lg shadow-lg hover:bg-emerald-600 transition-colors px-[11px] py-[6px]"
            >
              <School size={14} />

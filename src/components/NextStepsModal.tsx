...
const NextStepsModal: React.FC<NextStepsModalProps> = ({
  ...
  return <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={`${isMobile ? 'w-[95vw] h-[85vh] max-w-none max-h-none m-2' : 'w-full max-w-lg max-h-[80vh] mx-auto'} rounded-2xl p-0 overflow-hidden flex flex-col border-0 shadow-2xl`}>
        
        {/* Header fixo */}
        <DialogHeader className="flex-shrink-0 p-5 pb-3 border-b bg-white rounded-t-2xl">
          <DialogTitle className={`text-center text-gray-800 font-bold ${isMobile ? 'text-lg' : 'text-xl'}`}>
          ...
          <div className={`space-y-4 ${isMobile ? 'space-y-3' : 'space-y-4'}`}>
            ...
            <div>
              ...
              <div className={`space-y-3 ${isMobile ? 'space-y-2' : 'space-y-3'}`}>
                {steps.map(step => <div key={step.number} className="flex items-start space-x-3">
                    <div className={`flex-shrink-0 bg-blue-100 rounded-full flex items-center justify-center ${isMobile ? 'w-6 h-6' : 'w-8 h-8'}`}>
        ...
        <DialogFooter className="flex-shrink-0 p-5 pt-3 border-t bg-white rounded-b-2xl">
          <div className={`flex gap-3 ${isMobile ? 'flex-col' : 'flex-row justify-end'} w-full`}>
            ...
            <Button onClick={onContinue} className={`bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors ${isMobile ? 'w-full h-11 text-sm' : 'px-6 h-11'}`}>
              Visualizar Material
            </Button>
          </div>
        </DialogFooter>
